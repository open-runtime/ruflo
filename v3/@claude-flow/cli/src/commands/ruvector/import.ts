 ...
 *
 * Usage:
 *   ruflo ruvector import --input memory-export.json
 *   ruflo ruvector import --from-memory
 *   ruflo ruvector import --input data.json --batch-size 100
 *
...
export const importCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (fromMemory) {
      ...
      output.printWarning('Note: Run "ruflo memory list --format json > memory-export.json" first');
      output.printInfo('Then use: ruflo ruvector import --input memory-export.json');
