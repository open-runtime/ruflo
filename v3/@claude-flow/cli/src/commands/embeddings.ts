...
const initCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // Create directories
      const configDir = path.join(process.cwd(), '.claude', 'ruflo');
...
const neuralCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const fs = await import('fs');
    const path = await import('path');
    const configPath = path.join(process.cwd(), '.claude', 'ruflo', 'embeddings.json');
    ...
    if (!neuralConfig.enabled) {
      ...
      output.writeln(output.dim('Configuration: .claude/ruflo/embeddings.json'));
