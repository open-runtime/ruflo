 ...
 * Uses .claude/ruflo/neural in the current working directory,
 * falling back to home directory
 */
function getDataDir(): string {
  const cwd = process.cwd();
  const localDir = join(cwd, '.claude', 'ruflo', 'neural');
  const homeDir = join(homedir(), '.claude', 'ruflo', 'neural');

  // Prefer local directory if project runtime exists
  if (existsSync(join(cwd, '.claude', 'ruflo'))) {
    return localDir;
  }
