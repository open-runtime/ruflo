...
if [ -f ".claude/ruflo/metrics/v3-progress.json" ]; then
  DOMAINS=$(jq -r '.domains.completed // 0' ".claude/ruflo/metrics/v3-progress.json" 2>/dev/null || echo "0")
  AGENTS=$(jq -r '.swarm.activeAgents // 0' ".claude/ruflo/metrics/v3-progress.json" 2>/dev/null || echo "0")
  DDD_PROGRESS=$(jq -r '.ddd.progress // 0' ".claude/ruflo/metrics/v3-progress.json" 2>/dev/null || echo "0")
fi

if [ -f ".claude/ruflo/security/audit-status.json" ]; then
  CVES_FIXED=$(jq -r '.cvesFixed // 0' ".claude/ruflo/security/audit-status.json" 2>/dev/null || echo "0")
fi

if [ -f ".claude/ruflo/metrics/performance.json" ]; then
  SPEEDUP=$(jq -r '.flashAttention.speedup // "1.0x"' ".claude/ruflo/metrics/performance.json" 2>/dev/null || echo "1.0x")
  MEMORY=$(jq -r '.memory.reduction // "0%"' ".claude/ruflo/metrics/performance.json" 2>/dev/null || echo "0%")
fi
