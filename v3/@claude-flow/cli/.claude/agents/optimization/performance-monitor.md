...
ruflo health-check --components ["swarm", "agents", "coordination"]

# Collect specific metrics
ruflo metrics-collect --components ["cpu", "memory", "network"]

# Monitor SLA compliance
ruflo sla-monitor --service swarm-coordination --threshold 99.9
```

### Alert Configuration
```bash
# Configure performance alerts
ruflo alert-config --metric cpu_usage --threshold 80 --severity warning

# Set up anomaly detection
ruflo anomaly-setup --models ["statistical", "ml", "time_series"]

# Configure notification channels
ruflo notification-config --channels ["slack", "email", "webhook"]
```
