...
ruflo hook pre-task --description "Implement authentication"

# Post-edit hook (auto-formats and stores in memory)
ruflo hook post-edit --file "src/auth.js" --memory-key "auth/login"

# Session end hook (saves state and metrics)
ruflo hook session-end --session-id "dev-session" --export-metrics
```
...
Examples:
  ruflo hook pre-edit --file "src/auth/login.js"
  ruflo hook pre-edit -f "config/db.js" --validate-syntax
  ruflo hook pre-edit -f "production.env" --backup-file --check-conflicts
```
...
```bash
ruflo hook pre-bash --command <cmd>
...
Examples:
  ruflo hook pre-bash -c "rm -rf /tmp/cache"
  ruflo hook pre-bash --command "docker build ." --estimate-resources
```
...
Examples:
  ruflo hook pre-task --description "Implement user authentication"
  ruflo hook pre-task -d "Continue API dev" --load-memory
  ruflo hook pre-task -d "Refactor codebase" --optimize-topology
```
...
```bash
ruflo hook pre-search --query <query>
...
Examples:
  ruflo hook pre-search -q "authentication middleware"
```
...
Examples:
  ruflo hook post-edit --file "src/components/Button.jsx"
  ruflo hook post-edit -f "api/auth.js" --memory-key "auth/login"
  ruflo hook post-edit -f "utils/helpers.ts" --train-patterns
```
...
```bash
ruflo hook post-bash --command <cmd>
...
Examples:
  ruflo hook post-bash -c "npm test" --update-metrics
```
...
Examples:
  ruflo hook post-task --task-id "auth-implementation"
  ruflo hook post-task -t "api-refactor" --analyze-performance
  ruflo hook post-task -t "bug-fix-123" --store-decisions
```
...
```bash
ruflo hook post-search --query <query> --results <path>
...
Examples:
  ruflo hook post-search -q "auth" -r "results.json" --train-patterns
```
...
```bash
ruflo hook mcp-initialized --swarm-id <id>
...
```bash
ruflo hook agent-spawned --agent-id <id> --type <type>
...
```bash
ruflo hook task-orchestrated --task-id <id>
...
```bash
ruflo hook neural-trained --pattern <name>
...
```bash
ruflo hook memory-sync --namespace <ns>
...
```bash
ruflo hook session-start --session-id <id>
...
```bash
ruflo hook session-restore --session-id <id>
...
Examples:
  ruflo hook session-restore --session-id "swarm-20241019"
  ruflo hook session-restore -s "feature-auth" --restore-memory
```
...
```bash
ruflo hook session-end [options]
...
Examples:
  ruflo hook session-end --session-id "dev-session-2024"
  ruflo hook session-end -s "feature-auth" --export-metrics --generate-summary
  ruflo hook session-end -s "quick-fix" --cleanup-temp
```
...
Examples:
  ruflo hook notify -m "Task completed" --level info
  ruflo hook notify -m "Critical error" --level error --broadcast
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "ruflo hook pre-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/current'"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "ruflo hook pre-bash --command '${tool.params.command}'"
        }]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "ruflo hook post-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/complete' --auto-format --train-patterns"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "ruflo hook post-bash --command '${tool.params.command}' --update-metrics"
        }]
      }
    ]
  }
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "ruflo hook pre-edit --file '${tool.params.file_path}' --auto-assign-agent --validate-syntax",
            "timeout": 3000,
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "ruflo hook pre-task --description '${tool.params.task}' --auto-spawn-agents --load-memory",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "ruflo hook pre-search --query '${tool.params.pattern}' --check-cache"
          }
        ]
      }
    ],
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "ruflo hook post-edit --file '${tool.params.file_path}' --memory-key 'edits/${tool.params.file_path}' --auto-format --train-patterns",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "ruflo hook post-task --task-id '${result.task_id}' --analyze-performance --store-decisions --export-learnings",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "ruflo hook post-search --query '${tool.params.pattern}' --cache-results --train-patterns"
          }
        ]
      }
    ],
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "ruflo hook session-start --session-id '${session.id}' --load-context"
          }
        ]
      }
    ],
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "ruflo hook session-end --session-id '${session.id}' --export-metrics --generate-summary --cleanup-temp"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "ruflo hook check-protected --file '${tool.params.file_path}'"
          }
        ]
      }
    ]
...
```javascript
// Hook command
ruflo hook pre-task --description "Build REST API"
...
```javascript
// Hook command
ruflo hook post-edit --file "api/auth.js"
...
```javascript
// Hook command
ruflo hook session-end --session-id "dev-2024"
...
for FILE in $FILES; do
  ...
  ruflo hook pre-edit --file "$FILE" --validate-syntax
  ...
  ruflo hook post-edit --file "$FILE" --auto-format
done
...
# Run quality checks
ruflo hook session-end \
  --generate-report \
  --export-metrics

# Verify quality thresholds
TRUTH_SCORE=$(ruflo metrics score --format json | jq -r '.truth_score')
...
ruflo hook session-start --session-id "fullstack-feature"
...
# Backend work
ruflo hook pre-edit --file "api/profile.js"
# ... implement backend ...
ruflo hook post-edit \
  --file "api/profile.js" \
...
ruflo hook pre-edit --file "components/Profile.jsx"
# ... implement frontend ...
ruflo hook post-edit \
...
ruflo hook session-start --session-id "debug-memory-leak"
...
ruflo hook pre-search --query "EventEmitter"
# ... search executes ...
ruflo hook post-search \
  --query "EventEmitter" \
  --cache-results
...
ruflo hook pre-task --description "Analyze code complexity"
# ... analysis ...
ruflo hook post-task \
  --task-id "analysis" \
  --store-decisions
...
for file in src/**/*.js; do
  ruflo hook pre-edit --file "$file" --backup-file
  # ... refactor ...
  ruflo hook post-edit \
    --file "$file" \
...
ruflo hook pre-edit --file "test.js" --debug

# Check hook execution logs
cat .claude/ruflo/logs/hooks-$(date +%Y-%m-%d).log

# Validate configuration
ruflo hook validate-config
```
...
- `ruflo init --hooks` - Initialize hooks system
- `ruflo hook --list` - List available hooks
- `ruflo hook --test <hook>` - Test specific hook
- `ruflo memory usage` - Manage memory
- `ruflo agent spawn` - Spawn agents
- `ruflo swarm init` - Initialize swarm
