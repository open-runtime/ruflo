...
ruflo actions generate-workflow \
  --analyze-codebase \
  --detect-languages \
  ...
  detect-and-build:
    ...
    steps:
      ...
      - name: Detect Languages
        id: detect
        run: |
          ruflo actions detect-stack \
            --output json > stack.json
            
      - name: Dynamic Build Matrix
        run: |
          ruflo actions create-matrix \
            --from stack.json \
            --parallel-builds
```
    ...
    steps:
      - name: Security Analysis Swarm
        run: |
          ...
          SECURITY_ISSUES=$(ruflo actions security \
            --deep-scan \
            --format json)
  ...
  progressive-deploy:
    ...
    steps:
      - name: Analyze Risk
        id: risk
        run: |
          ruflo actions deploy-risk \
            --changes ${{ github.sha }} \
            --history 30d
            
      - name: Choose Strategy
        run: |
          ruflo actions deploy-strategy \
...
jobs:
  generate-matrix:
    ...
    steps:
      - id: set-matrix
        run: |
          MATRIX=$(ruflo actions test-matrix \
            --detect-frameworks \
            --optimize-coverage)
...
ruflo actions parallel-strategy \
...
ruflo actions failure-patterns \
  --period 90d \
  --classify-failures \
  --suggest-preventions
```
    ...
    steps:
      - name: Multi-Agent Validation
        run: |
          ...
          RESULTS=$(ruflo actions pr-validate \
