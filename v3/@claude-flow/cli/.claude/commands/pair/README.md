...
ruflo pair --start --agent senior-dev

# Start with verification
ruflo pair --start --verify --threshold 0.98
```
...
```bash
ruflo pair --start --mode switch --interval 10m
```
...
ruflo pair --start --git --review-commit
```

### With Testing Frameworks
```bash
# Jest integration
ruflo pair --start --test-framework jest

# Pytest integration
ruflo pair --start --test-framework pytest
```
...
Configure pair programming in `.claude/ruflo/config.json`:
