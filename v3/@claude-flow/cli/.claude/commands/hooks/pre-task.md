...
```bash
ruflo hook pre-task -d "Continue API development" --load-memory
```

### Manual agent control

```bash
ruflo hook pre-task -d "Debug issue #123" --auto-spawn-agents false
```

### Full optimization

```bash
ruflo hook pre-task -d "Refactor codebase" --optimize-topology --estimate-complexity
```
...
ruflo hook pre-task --description "Your task here"
```
