...
```bash
ruflo auto agent --task "Build a REST API with authentication"
```

### Constrained spawning

```bash
ruflo auto agent -t "Debug performance issue" --max-agents 3
```

### Analysis only

```bash
ruflo auto agent -t "Refactor codebase" --no-spawn
```

### Minimal strategy

```bash
ruflo auto agent -t "Fix bug in login" -s minimal
```
