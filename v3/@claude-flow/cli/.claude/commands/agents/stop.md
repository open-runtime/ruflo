...
```bash
ruflo agent stop <agent-id> [options]
ruflo agent kill <agent-id> [options]  # Alias
```
...
ruflo agent stop coder-lx7m9k2

# Force stop (immediate termination)
ruflo agent stop coder-lx7m9k2 --force

# Custom shutdown timeout
ruflo agent stop coder-lx7m9k2 --timeout 60

# Using kill alias
ruflo agent kill researcher-abc123 -f
```
...
ruflo agent list -t coder --format json | \
  jq -r '.agents[].id' | \
  xargs -I {} ruflo agent stop {} -f

# Stop all idle agents
ruflo agent list -s idle --format json | \
  jq -r '.agents[].id' | \
  xargs -I {} ruflo agent stop {}
```

## Related Commands

- `ruflo agent list` - Find agent IDs
- `ruflo agent status` - Check status before stopping
- `ruflo swarm destroy` - Stop all agents in swarm
