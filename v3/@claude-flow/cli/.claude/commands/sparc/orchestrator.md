...
```bash
# Initialize orchestration swarm
ruflo swarm init --topology hierarchical --strategy auto --max-agents 8

# Spawn coordinator agent
ruflo agent spawn --type coordinator --capabilities "task-planning,resource-management"

# Orchestrate tasks
ruflo task orchestrate --task "feature development" --strategy parallel --deps "auth,ui,api"
```
...
```bash
# 1. Initialize orchestration swarm
ruflo swarm init --topology hierarchical --max-agents 10

# 2. Create workflow
ruflo workflow create --name "feature-development" --steps "design,implement,test,deploy"

# 3. Execute orchestration
ruflo sparc run orchestrator "develop user management system" --parallel --monitor
